const API_KEY = 'b849878ba48a88e11a7eca2f0933c9eb';
...
function exibeFilmesEmCartaz() {
    ...
    for (i = 0; i < 10; i++) {
        ...
        xhrAvaliacoes.open('GET', `${API_URL}movie/${filme.id}/reviews?api_key=${API_KEY}&page=1`);
        xhrAvaliacoes.send();
    };
...
document.getElementById("destaque-categorias").addEventListener("change", (e) => {
    ...
    xhrDestaque.open('GET', `${API_URL}discover/movie?with_genres=${e.target.value}&api_key=${API_KEY}&language=pt-BR&page=1`);
    xhrDestaque.send();
...
document.querySelector(".mais-destaques").addEventListener("click", () => {
    ...
    xhrDestaque.open('GET', `${API_URL}discover/movie?with_genres=${document.getElementById("destaque-categorias").value}&api_key=${API_KEY}&language=pt-BR&page=1`);
    xhrDestaque.send();
});
...
function buscaInformacoesIniciais() {
    ...
    xhrGeneros.open('GET', `${API_URL}genre/movie/list?api_key=${API_KEY}&language=pt-BR`);
    xhrGeneros.send();

    let xhrEmCartaz = new XMLHttpRequest();
    xhrEmCartaz.onload = exibeFilmesEmCartaz;
    xhrEmCartaz.open('GET', `${API_URL}movie/now_playing?api_key=${API_KEY}&language=pt-BR&page=1`);
    xhrEmCartaz.send();

    let xhrDestaque = new XMLHttpRequest();
    xhrDestaque.onload = exibeFilmesEmDestaque;
    xhrDestaque.open('GET', `${API_URL}discover/movie?with_genres=28&api_key=${API_KEY}&language=pt-BR&page=1`);
    xhrDestaque.send();
}
