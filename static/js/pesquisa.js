...
const IMG_URL = 'http://image.tmdb.org/t/p/w342';
const LOGO_URL = 'http://image.tmdb.org/t/p/w45';
    ...
    for (i = 0; i < lista.results.length; i++) {
        ...
        switch (registro.media_type) {
            case "movie":
                ...
                texto += `
					<a href="detalhes.html?id=${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-12 col-md-1">
								<img src="${registro.poster_path?IMG_URL+registro.poster_path:DEFAULT_IMAGE}">
							</div>
            ...
            case "tv":
                ...
                texto += `
					<a href="${SITE_URL}tv/${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-12 col-md-1">
								<img src="${registro.poster_path?IMG_URL+registro.poster_path:DEFAULT_IMAGE}">
							</div>
            ...
            case "person":
                ...
                texto += `
					<a href="${SITE_URL}person/${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-12 col-md-1">
								<img src="${registro.profile_path?IMG_URL+registro.profile_path:DEFAULT_PROFILE}">
							</div>
							<div class="col-12 col-md-11">	
								<div class="d-flex w-100 justify-content-between">
									<h5 class="mb-1">${registro.name}</h5>
...
xhrPesquisa.open('GET', `${API_URL}search/multi?query=${query}&api_key=${API_KEY}&language=pt-BR&page=${page?page:1}&include_adult=false`);
xhrPesquisa.send();
